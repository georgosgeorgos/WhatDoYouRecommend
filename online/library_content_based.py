...
def userRecommendations(user_number, books_dataFrame, recommendation):
    ...
    for isbn in recommendation[user_number]:
        ...
        try:
            print(
                "<",
                books_dataFrame.loc[isbn]["Book-Title"],
                "> written by <",
                books_dataFrame.loc[isbn]["Book-Author"],
                ">",
            )
        except:
            continue
...
def mainContentBased(featuresDict, userDict, isbnDict, users, books, d_user, d_isbn, books_dataFrame, mostRated, t):
    ...
    if r == 1:
        ...
        if recommendation[user_number] == []:
            print(
                "We don't have a recommendation for user <",
                user_number,
                ">. We can only recommend the most popular and well rated books in the database:\n",
            )
            for i in mostRated:
                print(
                    "<",
                    books_dataFrame["Book-Title"][i[0]],
                    "> written by <",
                    books_dataFrame["Book-Author"][i[0]],
                    ">",
                )
            ...
            while r not in list(mostRatedDict.keys()):
                for m in mostRatedDict:
                    print(
                        m,
                        "<",
                        books_dataFrame["Book-Title"][mostRatedDict[m]],
                        "> written by <",
                        books_dataFrame["Book-Author"][mostRatedDict[m]],
                        ">",
                    )
                r = int(input())
            ...
            while r not in list(recDict.keys()):
                for m in recDict:
                    print(
                        m,
                        "<",
                        books_dataFrame["Book-Title"][recDict[m]],
                        "> written by <",
                        books_dataFrame["Book-Author"][recDict[m]],
                        ">",
                    )
                r = int(input())
            isbn = recDict[r]
